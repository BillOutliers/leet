...
思路：
  把dict中的内容添加到set中，便于查找
  把sentence根据“ ”分成若干个单词
  对每个单词中的前缀在set中进行查找，如果找到则进行修改
  重组句子
*/
...
public:
    string replaceWords(vector<string>& dict, string sentence) {
		...
		for(int i=0,j=0;i<sentence.size() && j<=sentence.size();j++)//把字符串分成单词
		{
		...
		{			
	        for(int j=0;j<mySentence[i].size();j++)
			{
				if(myDict.find(mySentence[i].substr(0,j+1)) != myDict.end())
				{
					mySentence[i]=mySentence[i].substr(0,j+1);
					break;
				}
			}
			res+=(mySentence[i]+" ");
